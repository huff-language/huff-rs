...
/// ### Codegen
///
/// Code Generation Manager responsible for generating bytecode from a [Contract](../../huff_utils/src/ast.rs#Contract) Abstract Syntax
/// Tree.
///
/// #### Usage
///
...
impl Codegen {
        ...
        for (ir_bytes_index, ir_byte) in ir_bytes.into_iter().enumerate() {
            ...
            match ir_byte.ty {
                ...
                IRByteType::Statement(s) => {
                    let push_bytes = statement_gen(&s, contract, &macro_def, scope, &mut offset, mis);
                }
    ...
    pub fn fill_unmatched(bytes: &Vec<(usize, Bytes)>, jump_table: &JumpTable, label_indices: &LabelIndices) -> Result<(Vec<(usize, Bytes)>, Vec<Jump>), CodegenError> {
